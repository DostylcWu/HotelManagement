...
public class CheckInServiceImpl implements ICheckInService {
	...
	public double judgeMoeny(int overTime, double money,
		 ...
		 if (overTime > 1) {
	            money = money + checkInBean.getRoom().getRoomType().getPrice()*checkInBean.getStayDays();
	            if (hotelDiscountBean != null) {
	                money = money * hotelDiscountBean.getValue();
	            }
	        } else {
	            money = checkInBean.getRoom().getRoomType().getPrice()*checkInBean.getStayDays();
	            if (hotelDiscountBean != null) {
	                money = money * hotelDiscountBean.getValue();
	            }
	        }
		...
		double resMoney = bd.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
        return resMoney;
