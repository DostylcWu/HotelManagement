...
<form action="/HotelManagement/managerAddReserve.action" method="post">
	<table>
		...
		<tr>
			<td>预订的房间类型1：</td>
			<td><input type="text" name="id" class="roomType"></td>
		</tr>
		<tr>
			<td>该种房间数量</td>
			<td><input type="text" name="count" class="roomCount"></td>
		</tr>
		<tr>
			<td>预订的房间类型2：</td>
			<td><input type="text" name="id" class="roomType"></td>
		...
		<tr>
			<td>入住人姓名(必填)</td>
			<td><input type="text" name="personone" id="checkInPerson"></td>
		</tr>
		<tr>
			<td>客户留言</td>
		...
		<tr>
			<td>抵达</td>
			<td><input type="text" name="arrive" id="arrive"></td>
		</tr>
		<tr>
			<td>取消</td>
	...
	<script type="text/javascript">	
		function addReserve() {
			...
			$(".roomType").each(function() {
				var roomType={};
				roomType.id=$(this).val();
				list.push({roomTypeBean:roomType});
				i++;
			});	
			...
			var person={};
			person.name=$("#checkInPerson").val();
			list.push({checkInPersonBean:person});
			
			var room={};
			room.checkIn=$("#checkIn").val();
			room.checkOut=$("#checkOut").val();
			room.cancel=$("#cancel").val();
			room.arrive=$("#arrive").val();
