...
public class OverTimeServiceImpl implements IOverTimeService{
    ...
    IRoomTypeDao tdao;
	public IHotelOverTimeDao getDao() {
		return dao;
	}

	public void setDao(IHotelOverTimeDao dao) {
		this.dao = dao;
	}
    
	public IRoomTypeDao getTdao() {
		return tdao;
	}

	public void setTdao(IRoomTypeDao tdao) {
		this.tdao = tdao;
	}
	...
	public String updateOverTimeRule(Date newOverHalfDayStartTime, Date newOverAllDayStartTime) {
		...
		try {
			...
			if(newOverHalfDayStartTime!=null){
				n=df.parse(df.format(newOverHalfDayStartTime));
			}
			if(newOverAllDayStartTime!=null){
				a=df.parse(df.format(newOverAllDayStartTime));
			}
			//如果两个时间一样或规定退房时间在超时一天的时间之后则修改失败
			if(n!=null&&a!=null&&(n.after(a)||n.equals(a))){
			return "修改失败！";
			}
			else{
